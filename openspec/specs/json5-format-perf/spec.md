...
Performance constraints for the JSON5 format pipeline: preserve byte-identical output while optimizing hot paths (notably sortKeys transform).
...
The format Document AST builder SHALL NOT require a separate token pre-index pass. Member-boundary lookups (`findCommaToken`, `streamNextToken`, `openLineEndToken`) SHALL use direct scans over the filled token stream.
...
Performance optimizations SHALL apply only to modules under `src/parser/json5/format/` and the top-level `src/parser/json5/format.js` orchestrator. They SHALL NOT modify `parse.js`, `validate.js`, shared `src/parser/core/` parse pipeline, or ANTLR grammars.
