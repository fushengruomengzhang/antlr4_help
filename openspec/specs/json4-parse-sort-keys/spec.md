...
`JSON4.parse` optional `sortKeys` option: when enabled, recursively sort plain object keys using locale-aware stable string comparison aligned with `JSON5.format` `sortKeys` semantics. Array element order is unchanged.
