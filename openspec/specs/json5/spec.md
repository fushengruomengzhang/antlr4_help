...
JSON5 validate, parse, and format: module layout, single format stack, parse sortKeys, Document AST pipeline with AnchorTriplet coordinates, and format performance constraints.
...
#### Scenario: Sort compact matches golden baseline

- **WHEN** `JSON5.format(input, { sortKeys: true, compact: true })` is called on the comment-heavy fixture
- **THEN** output SHALL match `test/resources/expected/json5/fixture.sort-compact.text` byte-for-byte
...
- **THEN** pretty/compact output SHALL preserve `// 三引号注释` on the opener line in the formatted string literal emission
...
- **THEN** comma and next-token coordinates SHALL match behavior validated by existing tests
...
`openspec/specs/json5/spec.zh.md` SHALL fully mirror `spec.md` in Simplified Chinese with the same preservation rules as the json4 documentation requirement.

#### Scenario: json5 mirror exists

- **WHEN** the repository is checked out
- **THEN** `openspec/specs/json5/spec.zh.md` SHALL exist
- **AND** it SHALL cover module layout, parse sortKeys, format AST/triplet, and perf requirements
