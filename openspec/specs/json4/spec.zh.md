...
标准 JSON（`JSON4`）parse 行为，含与 `JSON5.format` key 排序对齐的可选 `sortKeys`。
...
### Requirement: 模块 spec 提供简体中文 mirror 文档

`openspec/specs/` 下每个模块 SHALL 在 `spec.md` 旁提供 `spec.zh.md`，完整 mirror 全部 requirement 与 scenario（简体中文），并保留英文 normative 关键字（`Requirement`、`Scenario`、`WHEN`、`THEN`、`AND`、`SHALL`、`MUST`、`MAY`、`NOT`）及全部代码字面量、路径与 API 标识符。

#### Scenario: json4 mirror 存在

- **WHEN** 检出仓库
- **THEN** `openspec/specs/json4/spec.zh.md` SHALL 存在
- **AND** 其 `### Requirement:` 数量 SHALL 与 `spec.md` 相同

#### Scenario: 英文 spec 仍为权威

- **WHEN** 运行 `openspec validate --all`
- **THEN** 校验 SHALL 仅适用于 `spec.md`，不适用 `spec.zh.md`
