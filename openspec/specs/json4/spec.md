...
Standard JSON (`JSON4`) parse behavior, including optional `sortKeys` aligned with `JSON5.format` key ordering.
...
Each module under `openspec/specs/` SHALL provide a `spec.zh.md` alongside `spec.md` that fully mirrors all requirements and scenarios in Simplified Chinese while preserving English normative keywords (`Requirement`, `Scenario`, `WHEN`, `THEN`, `AND`, `SHALL`, `MUST`, `MAY`, `NOT`) and all code literals, paths, and API identifiers.
