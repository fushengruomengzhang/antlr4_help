...
The format Document AST builder SHALL precompute token navigation indexes in a single O(n) pass over the filled token stream at the start of `buildDocumentAst`, and SHALL use those indexes for member-boundary lookups instead of repeated per-member linear scans equivalent to the current `findCommaToken`, `streamNextToken`, and `openLineEndToken` behavior.

#### Scenario: Comma lookup equivalent to linear scan

- **WHEN** building an object or array with members separated by commas and hidden-channel tokens
- **THEN** comma token resolution for each member boundary SHALL produce the same token coordinates as the pre-optimization implementation

#### Scenario: Next default-channel token equivalent

- **WHEN** resolving the token after a key or primitive value during AST build
- **THEN** the next DEFAULT channel token SHALL match the pre-optimization `streamNextToken` result

#### Scenario: Open-line end token equivalent

- **WHEN** resolving layout anchors after `{` or `[` on a line with inline hidden tokens
- **THEN** the open-line end coordinate SHALL match the pre-optimization `openLineEndToken` result
