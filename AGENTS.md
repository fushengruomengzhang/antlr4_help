...
- `src/grammars/*.g4` → ANTLR grammar sources; `src/parser/` → generated JS parser (committed); `src/index.js` → ESM entrypoint. `lib/antlr-4.9.3-complete.jar` is the Java generator tool. Standard commands live in `package.json` and `README.md`: `npm install`, `npm run generate`, `npm start` / `node src/index.js [input]`.
...
- `src/parser/` generated code is committed. After editing any `src/grammars/*.g4`, you MUST re-run `npm run generate` and commit the regenerated `src/parser/` — otherwise generated code drifts from the grammar.
